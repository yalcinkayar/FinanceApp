...
@section('content')
    <div class="row page-title clearfix">
        ...
        <div class="page-title-right d-none d-sm-inline-flex">
            ...
            <div class="d-none d-md-inline-flex justify-center align-items-center"><a href="javascript: void(0);" class="btn btn-color-scheme btn-sm fs-11 fw-400 mr-l-40 pd-lr-10 mr-l-0-rtl mr-r-40-rtl hidden-xs hidden-sm ripple" target="_blank">Update Billing</a>
            </div>
        </div>
        <!-- /.page-title-right -->
    </div>
            ...
            <div class="col-md-12 widget-holder">
                <div class="widget-bg">
                    <div class="widget-body clearfix">
                        <form action="{{ route('billing.update',['id'=>$data[0]['id']]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group row firma--area">
                                <div class="col-md-4">
                                    <label class="col-form-label" for="l0">Billing No</label>
                                    <input class="form-control" required name="billingno" type="text" value="{{$data[0]['billingno']}}">
                                </div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input class="form-control" required name="billingdate" type="date" value="{{$data[0]['billingdate']}}">
                                </div>
                                        ...
                                        <table id="billingData" class="table">
                                            ...
                                            @foreach($operation as $k => $v)
                                                <tr class="operation_field">
                                                    <td style="width: 150px;"><select class="form-control item" name="operation[{{$k}}][itemid]">
                                                            <option value="0">Select Item</option>
                                                            @foreach(\App\Item::getList(1) as $key => $value)
                                                                <option @if($value['id'] == $v['itemid']) selected @endif data-k ="{{$value['tax']}}" value="{{ $value['id'] }}">{{ $value['name'] }}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <td style="width: 180px;">
                                                        <select class="form-control product" name="operation[{{ $k }}][productid]">
                                                            <option value="0">Select Product</option>
                                                            @foreach(\App\Products::all() as $key => $value)
                                                                <option @if($value['id'] == $v['productid']) selected @endif data-saleprice ="{{$value['purchasePrice']}}" value="{{ $value['id'] }}">{{ $value['productName'] }}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>

                                                <td><input type="text" class="form-control" id="day_quantity" name="operation[{{$k}}][day_quantity]" value="{{$v['amount']}}"></td>
                                                <td><input type="text" class="form-control" id="price" name="operation[{{$k}}][price]" value="{{$v['price']}}"></td>
                                                <td><input type="text" class="form-control" id="subtotal" name="operation[{{$k}}][subtotal]" value="{{$v['subtotal']}}"></td>
                                                <td><input type="text" class="form-control" id="tax" name="operation[{{$k}}][tax]" value="{{$v['tax']}}"></td>
                                                <td><input type="text" class="form-control" id="total_taxed" name="operation[{{$k}}][total_taxed]" value="{{$v['total_taxed']}}"></td>
                                                <td><input type="text" class="form-control" id="overalltotal" name="operation[{{$k}}][overalltotal]" value="{{$v['overalltotal']}}"></td>
                                                <td><input type="text" class="form-control" id="text" name="operation[{{$k}}][text]" value="{{$v['text']}}"></td>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#addRowBtn").click(function () {
                var newRow =
                    '<tr class="operation_field">' +
                    '<td><select class="form-control item" name="operation[' + i + '][itemid]">' +
                    '<option value="0">Select Item</option>';
                @foreach(\App\Item::getList(1) as $key => $value)
                ...
                @endforeach
                    ...
                    '<option value="0">Select Product</option>';
                @foreach(\App\Products::all() as $key => $value)
                    newRow+='<option data-purchaseprice ="{{$value['purchasePrice']}}" value="{{ $value['id'] }}">{{ $value['productName'] }}</option>';
                @endforeach
                    newRow += '</select></td>' +
            ...
            $("body").on("change",".product",function () {
                var purchaseprice = $(this).find(":selected").data("purchaseprice");
                $(this).closest(".operation_field").find("#price").val(purchaseprice);
            });
